...
//attribute
double calculate_gain(vector <vector <string> > remain_state, string attribute){
	unsigned int j,k,m;
	//
...
Node * Decision_tree(Node * p, vector <vector <string> > remain_state, vector <string> remain_attribute){
	...
	for(it1 = remain_attribute.begin(); it1 < remain_attribute.end(); it1++){
		temp_gain = calculate_gain(remain_state, (*it1));
	...
	for(vector <string>::iterator it3 = values.begin(); it3 < values.end(); it3++){
		...
		if(new_state.size() == 0){//?new_node
			...
			Decision_tree(new_node, new_state, new_attribute);
		//1  2new_state
...
int main(){
	...
	map_with_vector();
	root = Decision_tree(root,remain_state,remain_attribute);
	PrintTree(root,0);
	cout<<endl;
