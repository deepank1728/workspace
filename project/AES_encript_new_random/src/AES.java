...
public class AES {
	...
	private static byte[][] generateSubkeys(byte[] key,int random_number) {
		byte[][] tmp = new byte[Nb * (Nr + 1)][4];
		byte [][] swap=new byte [1][4];
		int i = 0;
		while (i < Nk) {
			tmp[i][0] = key[i * 4];
			tmp[i][1] = key[i * 4 + 1];
		...
		for(int j=0;j<4;j++){
			tmp[0][j]=tmp[random_number][j];
		}
		
		for(int j=0;j<4;j++){
			tmp[random_number][j]=swap[0][j];
		}
	...
	public static byte[] encrypt(String textClar,byte[] in,byte[] key,int random_number){
		...
		w = generateSubkeys(key,random_number);
	...
	public static byte[] decrypt(byte[] in,byte[] key,int random_number){
		int i;
		byte[] tmp = new byte[in.length];
		byte[] bloc = new byte[16];
		
		if(deepank.equals(in))
			System.out.println("same in and deepank");
		Nb = 4;
		Nk = key.length/4;
		Nr = Nk + 6;
		w = generateSubkeys(key,random_number);
